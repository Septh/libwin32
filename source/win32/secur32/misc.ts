...
export function GetUserNameEx(NameFormat: EXTENDED_NAME_FORMAT): string | null {
    GetUserNameEx.fn ??= secur32.func('GetUserNameExW', cBOOL, [ cINT, cLPWSTR, koffi.inout(cLPDWORD) ])

    const out = new Uint16Array(UNLEN)
    const len: OUT<number> = [ out.length ]
    return GetUserNameEx.fn(NameFormat, out, len) === 0
        ? null
        : textDecoder.decode(out.subarray(0, len[0]))   // -GetUserNameEx() does *not* includes the final \0
}

/** @internal */
export declare namespace GetUserNameEx {
    export var fn: koffi.KoffiFunc<(NameFormat: number, lpBuffer: Uint16Array, pcbBuffer: OUT<number>) => number>
}
