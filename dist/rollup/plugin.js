...
export function koffi() {
    // The path to Koffi's .node file.
    const koffiPath = fileURLToPath(new URL(`./build/koffi/${platform}_${arch}/koffi.node`, import.meta.resolve('koffi') // Unflagged in Node v20.6.0, v18.19.0
    ));
    ...
    return {
        ...
        async buildStart() {
            ...
            const access = await fs.access(koffiPath, fs.constants.R_OK).catch((err) => err);
